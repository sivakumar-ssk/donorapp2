...
def check(email):
    url = "https://axhiimolub.execute-api.us-east-2.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://axhiimolub.execute-api.us-east-2.amazonaws.com/plasma/registration160?"+params
...
@app.route('/stats')
def stats():
    url = "https://axhiimolub.execute-api.us-east-2.amazonaws.com/plasma/getbloodgroupsdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://axhiimolub.execute-api.us-east-2.amazonaws.com/plasma/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
    for i in a:
        url="https://www.fast2sms.com/dev/bulk?authorization=YOUR_API_KEY&sender_id=FSTSMS&message=Need Plasma of Your Blood Group&language=english&route=p&numbers="+str(i['phone'])
